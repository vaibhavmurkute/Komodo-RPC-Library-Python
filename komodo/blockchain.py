...
def coinsupply(height=0):
    ...
    data = '{'+rpc.get_request_metadata()+', ' \
                '"method": "coinsupply", ' \
                '"params": [' + (str(height) if height > 1 else '')+']' \
            '}'
...
def getblockhashes(high=0, low=0, no_orphans=False, logical_times=False):
    ...
    '''
    options = '{"noOrphans":'+str(no_orphans).lower()+\
                ', "logicalTimes":'+str(logical_times).lower()+'}'
    data = '{'+rpc.get_request_metadata()+', ' \
                '"method": "getblockhashes", ' \
                '"params": ['+str(high)+', '+str(low)+', '+options+'] ' \
            '}'
    return rpc.rpc_request(data)
...
def getblockheader(hash='', verbose=True):
    ...
    data = '{'+rpc.get_request_metadata()+', ' \
                ...
                '"params": ["'+str(hash)+'", '+str(verbose).lower()+'] ' \
            '}'
    return rpc.rpc_request(data)
...
def gettxout(txid='', vout=0, includemempool=False):
    ...
    data = '{'+rpc.get_request_metadata()+', ' \
                '"method": "gettxout", ' \
                '"params": ["'+str(txid)+'", '+str(vout)+', '+\
                    str(includemempool).lower()+'] ' \
            '}'
    return rpc.rpc_request(data)
...
def gettxoutproof(txid=[''], blockhash=''):
    ...
    data = '{'+rpc.get_request_metadata()+', ' \
                ...
                '"params": ['+txid_list+ \
                    (', "'+str(blockhash)+'"' if blockhash != '' else '')+']' \
            '}'
    return rpc.rpc_request(data)
...
def notaries(height=0, timestamp=0):
    ...
    :return: JSON string containing:
        ...
        "timestamp" (number) the timestamp at which the notary-node
            information applies
    '''
    options = ('"'+str(height)+'", "'+str(timestamp)+
                '"' if(height != 0 and timestamp != 0) else('"'+str(height)+
                    '"' if height > 0 else(('"'+str(timestamp)+
                    '"' if timestamp > 0 else ''))))
    data = '{'+rpc.get_request_metadata()+', ' \
                '"method": "notaries", ' \
                '"params": ['+options+'] ' \
...
def verifychain(checklevel=3, numblocks=0):
    ...
    data = '{'+rpc.get_request_metadata()+', ' \
                '"method": "verifychain", ' \
                '"params": ['+str(checklevel)+','+str(numblocks)+'] ' \
            '}'
    return rpc.rpc_request(data)
