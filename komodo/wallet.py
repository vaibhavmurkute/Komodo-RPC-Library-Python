...
def encryptwallet(passphrase=''):
    ...
    :param passphrase:  (string)	the passphrase for wallet encryption; the passphrase must be at least 1 character, but should be many
    :return:    (String)    wallet encrypted; Komodo server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.
    '''
    data = '{'+rpc.get_request_metadata()+', "method": "encryptwallet", "params": ['+str(passphrase)+'] }'
    return rpc.rpc_request(data)
...
def getbalance(account='', minconf=1, includeWatchOnly=False):
    '''
    The getbalance method returns the server's total available balance.The account input is deprecated.
    :param account:(string, optional)	DEPRECATED if provided, it MUST be set to the empty string "" or to the string "*"
    :param minconf:(numeric, optional, default=1)	only include transactions confirmed at least this many times
    :param includeWatchOnly:(bool, optional, default=false)	also include balance in watchonly addresses
...
def getnewaddress(account=''):
    ...
    :param account:(string, optional)	DEPRECATED: If provided, the account MUST be set to the empty string "" to represent the default account
...
def getreceivedbyaddress(address='', minconf=1):
    ...
    :param minconf:(numeric, optional, default=1)	only include transactions confirmed at least this many times
    :return: amount	(JSON string)	the total amount of the relevant coin received at this address
    '''
    data = '{'+rpc.get_request_metadata()+', "method": "getreceivedbyaddress", "params": ["' + str(address) + '", ' + str(minconf) + '] }'
    return rpc.rpc_request(data)
...
def gettransaction(txid, includeWatchOnly=False):
    ...
    :param includeWatchOnly: (bool, optional, default=false)	whether to include watchonly addresses in the returned balance calculation and in the details[] returned values
    :return: JSON string with transaction details
    '''
    data = '{'+rpc.get_request_metadata()+', "method": "gettransaction", "params": ["' + str(txid) + ', ' + str(includeWatchOnly) + '"] }'
    return rpc.rpc_request(data)
...
def importaddress(address, label='', rescan=True):
    ...
    :return: JSON string
    '''
    data = '{'+rpc.get_request_metadata()+', "method": "importaddress", "params": ["' + str(address) + '", "' + str(label) + '", ' + str(rescan).lower() + '] }'
    return rpc.rpc_request(data)
...
def listreceivedbyaddress(minconf=1, includeEmpty=False, includeWatchOnly=False):
    ...
    :return:JSON string:
        ...
        "confirmations"	(numeric)	a confirmation number that is dPoW aware; see this article for more info
    ...
    '''
    data = '{'+rpc.get_request_metadata()+', "method": "listreceivedbyaddress", "params": [' + str(minconf) + ', ' + str(includeEmpty).lower() + ', ' + str(includeWatchOnly).lower() + '] }'
    return rpc.rpc_request(data)
...
def listsinceblock(blockhash='', targetconf=1, includeWatchOnly=False):
    ...
    :param blockhash: (string, optional)	the block hash from which to list transactions
    :param targetconf: (numeric, optional)	the confirmations required (must be 1 or more)
    :param includeWatchOnly: (bool, optional, default=false)	include transactions to watchonly addresses
...
def listtransactions(account="*", count=10, skip=0, includeWatchOnly=False):
    ...
    :param account: (string, optional)	DEPRECATED the account name; should be "*"
    :param count:(numeric, optional, default=10)	the number of transactions to return
    :param skip: (numeric, optional, default=0)	the number of transactions to skip
    :param includeWatchOnly: (bool, optional, default=false)	include transactions to watchonly addresses
...
def listunspent(minconf=1, maxconf=9999999, addresses=[""]):
    '''
    The listunspent method returns an array of unspent transaction outputs, with a range between minconf and maxconf (inclusive) confirmations
    :param minconf:(numeric, optional, default=1)	the minimum confirmations to filter
    :param maxconf:(numeric, optional, default=9999999)	the maximum confirmations to filter
    ...
    for addr in addresses:
        # it seems Komodo rpc considers ' and " differently. So instead of just str(addresses), have to change ' to "
        addr_list += "\"" + str(addr) + "\","
    if (len(addresses) > 0):
...
def lockunspent(unlock=False, txid='', vout=0):
    ...
    '''
    data = '{'+rpc.get_request_metadata()+', "method": "lockunspent", "params": [' + str(unlock).lower() + ', [{"txid":"' + str(txid) + '","vout":' + str(vout) + '}]] }'
    return rpc.rpc_request(data)
...
def resendwallettransactions():
    ...
    :return: "transaction_id"	(JSON string)	an array of the rebroadcasted transaction id's
    '''
    data = '{'+rpc.get_request_metadata()+', "method": "resendwallettransactions", "params": [] }'
    return rpc.rpc_request(data)

def sendmany(account='', amounts={'': 0}, minconf=1, comment='', subtractFeeFromAmt=['']):
    '''
    The sendmany method can send multiple transactions at once. Amounts are double-precision floating point numbers
    :param account:(string, required)	MUST be set to the empty string "" to represent the default account; passing any other string will result in an error
    :param amounts:{"string":numeric}	dictionary with the address (string) and the value (double-precision floating numeric)
    :param minconf:(numeric, optional, default=1)	only use the balance confirmed at least this many times
    :param comment: (string, optional)	a string comment
    :param subtractFeeFromAmt: [string, optional] (string, optional) a string list with addresses. The fee will be equally deducted from the amount of each selected address; the recipients will receive less than you enter in their corresponding amount field.
    :return: "transaction_id"	(JSON string)	the transaction id for the send; only 1 transaction is created regardless of the number of addresses
    '''
    amount_list = "{";
    for addr in amounts:
    ...
    data = '{'+rpc.get_request_metadata()+', "method": "sendmany", "params": ["", ' + amount_list + ', ' + str(minconf) + ', "' + str(comment) + '", '+subtractFeeFrom_list+'] }'
    print(data)
    return rpc.rpc_request(data)
...
def sendtoaddress(address='', amount=0.0, comment='', comment_to='', subtractFeeFromAmt=False):
    ...
    :param amount: (numeric, required)	the amount to send (json requires all decimals values less than 1 begin with the characters '0.')
    ...
    :param comment_to: (string, optional)	a comment to store the name of the person or organization to which you're sending the transaction; this is stored in your local wallet file only
    :param subtractFeeFromAmt: (boolean, optional, default=false)	when true, the fee will be deducted from the amount being sent
    :return: "transaction_id"	(JSON string)	the transaction id
    '''
    data = '{'+rpc.get_request_metadata()+', "method": "sendtoaddress", "params": ["' + str(address) + '", ' + str(amount) + ', "' + str(comment) + '", "' + str(comment_to) + '", ' + str(subtractFeeFromAmt).lower() + '] }'
    return rpc.rpc_request(data)
...
def settxfee(amount=0.0):
    ...
    :param amount: (numeric, required)	the transaction fee in COIN/kB rounded to the nearest 0.00000001
...
def signmessage(address='', message=''):
    ...
    :param address: (string, required)	the address to use for the private key
    :param message:(string, required)	the message
    :return: "signature"	(JSON string)	the signature of the message encoded in base 64
    '''
...
def walletlock():
    ...
    :return: JSON string
    '''
    data = '{' + rpc.get_request_metadata() + ', "method": "walletlock", "params": [] }'
    return rpc.rpc_request(data)
...
def walletpassphrase(passphrase='', timeout=0):
    ...
    :param passphrase: (string)	the passphrase that was set by the encryptwallet method
    :param timeout:(number in seconds, optional)	the amount of time for which the wallet should remember the passphrase
    :return: JSON string
    '''
    data = '{' + rpc.get_request_metadata() + ', "method": "walletpassphrase", "params": ["'+str(passphrase)+'", '+str(timeout)+'] }'
    return rpc.rpc_request(data)
...
def walletpassphrasechange(oldpassphrase='', newpassphrase=''):
    ...
    :return: JSON string
    '''
    data = '{' + rpc.get_request_metadata() + ', "method": "walletpassphrasechange", "params": ["'+str(oldpassphrase)+'", "'+str(newpassphrase)+'"] }'
    return rpc.rpc_request(data)

def z_exportkey(z_address=''):
    '''
    The z_exportkey method reveals the private z_key corresponding to z_address
    :param z_address:(string, required)	the z_address for the private key
...
def z_exportwallet(filename='z_wallet_export'):
    ...
    :param filename:(string, required)	the filename, saved to the directory indicated by the exportdir parameter at daemon runtime (required)
    :return: "path"	(JSON string)	the full path of the destination file
    '''
...
def z_getbalance(address='', minconf=1):
    ...
    :param minconf: (numeric, optional, default=1)	only include transactions confirmed at least this many times
    :return: amount	(JSON numeric)	the total amount received at this address (in the relevant COIN value)
    '''
...
def z_getoperationresult(operationid=['']):
    ...
    :param operationid: [string, optional]	a list of operation ids to query; if not provided, the method examines all operations known to the node
    :return: JSON String
...
def z_getoperationstatus(operationid=['']):
    ...
    :param operationid: [string, optional]	a list of operation-ids we are interested in; if an array is not provided, the method examines all operations known to the node
...
def z_gettotalbalance(minconf=1, includeWatchOnly=False):
    ...
    :param minconf: (numeric, optional, default=1)	only include private and transparent transactions confirmed at least this many times
    :param includeWatchOnly: (bool, optional, default=false)	also include balance in watchonly addresses
    :return: JSON string:
...
def z_importkey(z_privatekey='', rescan='whenkeyisnew', startHeight=0):
    '''
    The z_importkey method imports z_privatekey to your wallet. This call can take minutes to complete if rescan is true
    :param z_privatekey: (string, required)	the z_privatekey
    :param rescan: (string, optional, default="whenkeyisnew")	rescan the wallet for transactions; can be yes
    :param startHeight: (numeric, optional, default=0)	the block height at which to begin the rescan
    :return: JSON string
    '''
...
def z_importviewingkey(viewing_key='', rescan='whenkeyisnew', startHeight=0):
    ...
    :param rescan: (string, optional, default="whenkeyisnew")	whether to rescan the wallet for transactions; can be "yes"
    :param startHeight: (numeric, optional, default=0)	block height to start rescan
    :return: JSON string
    '''
...
def z_listaddresses(includeWatchOnly=False):
    ...
    :param includeWatchOnly: (bool, optional, default=false)	also include watchonly addresses
    :return: "z_address"	( JSON string)	a z address belonging to the wallet
    '''
...
def z_listoperationids(status=''):
    ...
    :param status: (string, optional)	filter result by the operation's state e.g. "success"
    :return: "operationid"	(JSON string)	an operation id belonging to the wallet
    '''
...
def z_listunspent(minconf=1, maxconf=9999999, includeWatchOnly=False, addresses=['']):
    ...
    :param minconf: (numeric, optional, default=1)	the minimum confirmations to filter
    :param maxconf: (numeric, optional, default=9999999)	the maximum confirmations to filter
    :param includeWatchOnly: (bool, optional, default=false)	whether to also include watchonly addresses
    :param addresses: [string] a list of z addresses (both Sprout and Sapling) to act as a filter; duplicate addresses are not allowed
    :return: JSON string
    '''
    addr_list = "[";
    for addr in addresses:
...
def z_mergetoaddress(fromaddresses=[''], toaddress='', fee=0.0001, transparent_limit=50, shielded_limit=10, memo=''):
    '''
    The z_mergetoaddress method merges multiple utxos and notes into a single utxo or note.
    :param fromaddresses: [string, required] a list of z or t addresses
    :param toaddress: (string, required)	the t address or z address to receive the combined utxo
    :param fee: (numeric, optional, default=0.0001)	the fee amount to attach to this transaction
    :param transparent_limit: (numeric, optional, default=50)	limit on the maximum number of transparent utxos to merge; you may set this value to 0 to use the node option mempooltxinputlimit
    :param shielded_limit: (numeric, optional, default=10)	limit on the maximum number of hidden notes to merge; you may set this value to 0 to merge as many as will fit in the transaction
    :param memo: (string, optional)	encoded as hex; when toaddress is a z address, this value will be stored in the memo field of the new note
    :return: JSON string:
...
def z_sendmany(fromaddress='', amounts={'': 0.0}, memo='', minconf=1, fee=0.0001):
    '''
    The z_sendmany method sends one or more transactions at once, and allows for sending transactions of types t --> t, t --> z, z --> z, z --> t.
    :param fromaddress: (string, required)	the sending t address or z address
    :param amounts: {string : numeric}  a dictionary of z or t address as key and numeric amout value to be sent
    :param memo: (string, optional)	if the address is a z address, this property accepts raw data represented in hexadecimal string format
    :param minconf: (numeric, optional, default=1)	only use funds confirmed at least this many times
    :param fee: (numeric, optional, default=0.0001)	the fee amount to attach to this transaction
    :return: "operationid"	(string)	an operationid to pass to z_getoperationstatus to get the result of the operation
    '''
    amount_list = '[';
    for addr in amounts:
...
def z_shieldcoinbase(fromaddress='', toaddress='', fee=0.0001, limit=50):
    ...
    :param fromaddress: (string, required)	the address is a t address or "*" for all t address belonging to the wallet
    :param toaddress: (string, required)	the address is a z address
    :param fee: (numeric, optional, default=0.0001)	the fee amount to attach to this transaction
    :param limit: (numeric, optional, default=50)	limit on the maximum number of utxos to shield; set to 0 to use node option mempooltxinputlimit
    :return: JSON string:
...
def zcbenchmark(benchmarktype='', samplecount=1):
    ...
    :return: "runningtime"	(numeric)	the time it took to run the selected benchmarktype
    '''
